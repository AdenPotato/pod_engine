...
void ImGuiLayer::OnEvent(Event& e)
{
    ...
    {
        ImGuiKey ImKey = ToImGuiKey(ev.GetKeyCode());
    ...
    dispatcher.Dispatch<MouseButtonPressedEvent>([&](MouseButtonPressedEvent& ev) {
        ImGui::GetIO().AddMouseButtonEvent(ev.GetMouseButton(), true);
        return ImGui::GetIO().WantCaptureMouse;
    });

    dispatcher.Dispatch<MouseButtonReleasedEvent>([&](MouseButtonReleasedEvent& ev) {
        ImGui::GetIO().AddMouseButtonEvent(ev.GetMouseButton(), false);
         return ImGui::GetIO().WantCaptureMouse;
    });

    dispatcher.Dispatch<MouseMovedEvent>([&](MouseMovedEvent& ev) {
        ImGui::GetIO().AddMousePosEvent(ev.GetX(), ev.GetY());
        return ImGui::GetIO().WantCaptureMouse;
    });

    dispatcher.Dispatch<MouseScrolledEvent>([&](MouseScrolledEvent& ev) {
        ImGui::GetIO().AddMouseWheelEvent(ev.GetXOffset(), ev.GetYOffset());
        return ImGui::GetIO().WantCaptureMouse;
    });
}
...
ImGuiKey ImGuiLayer::ToImGuiKey(int key)
{
    switch (key)
    {
